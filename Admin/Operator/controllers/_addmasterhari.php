...
// Cek apakah ada data yang dikirimkan melalui POST
    // Generate kd_hari di backend
    $kd_hari = $acakangka4only;
    $nm_hari = trim($_POST['nm_hari']); // Menghapus spasi di awal dan akhir

    // Validasi jika nm_hari kosong
    if (empty($nm_hari)) {
        $response = array('status' => 'error', 'message' => 'Harap isi level user.');
    } else {
        // Cek apakah kd_hari sudah ada di database
        $qryData = $mysqli->query("SELECT * FROM dt_hari WHERE kd_hari = '$kd_hari'")
        ...
        if ($cekBaris) {
            ...
            $insData = $mysqli->query("INSERT INTO dt_hari (kd_hari, nm_hari) VALUES('$kd_hari', '$nm_hari')");
