...
// Validasi apakah data 'kd_MasterjamPP' ada dan tidak kosong
if (!isset($input['kd_MasterjamPP']) || empty($input['kd_MasterjamPP'])) {
    $response = array('status' => 'error', 'message' => 'Kode Navbar harus diisi.');
} else {
    $kd_MasterjamPP = trim($input['kd_MasterjamPP']);

    // Prepared Statement untuk menghindari SQL Injection
    if ($stmt = $mysqli->prepare("DELETE FROM dt_masterjampp WHERE kd_MasterjamPP = ?")) {
        $stmt->bind_param('s', $kd_MasterjamPP);
