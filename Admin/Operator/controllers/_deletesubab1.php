...
// Validasi apakah data 'kd_subab1nav2' ada dan tidak kosong
if (!isset($input['kd_subab1nav2']) || empty($input['kd_subab1nav2'])) {
    $response = array('status' => 'error', 'message' => 'Kode Subab harus diisi.');
} else {
    $kd_subab1nav2 = trim($input['kd_subab1nav2']);

    // Prepared Statement untuk menghindari SQL Injection
    if ($stmt = $mysqli->prepare("DELETE FROM dt_subab1nav2 WHERE kd_subab1nav2 = ?")) {
