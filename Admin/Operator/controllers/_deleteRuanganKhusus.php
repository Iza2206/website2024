...
// Validasi apakah data 'kd_RuanganKhusus' ada dan tidak kosong
if (!isset($input['kd_RuanganKhusus']) || empty($input['kd_RuanganKhusus'])) {
    $response = array('status' => 'error', 'message' => 'Kode Navbar harus diisi.');
} else {
    $kd_RuanganKhusus = trim($input['kd_RuanganKhusus']);

    // Prepared Statement untuk menghindari SQL Injection
    if ($stmt = $mysqli->prepare("DELETE FROM dt_ruangankhusus WHERE kd_RuanganKhusus = ?")) {
        $stmt->bind_param('s', $kd_RuanganKhusus);
