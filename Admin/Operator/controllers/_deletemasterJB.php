...
// Validasi apakah data 'kd_MasterJB' ada dan tidak kosong
if (!isset($input['kd_MasterJB']) || empty($input['kd_MasterJB'])) {
    $response = array('status' => 'error', 'message' => 'Kode Navbar harus diisi.');
} else {
    $kd_MasterJB = trim($input['kd_MasterJB']);

    // Prepared Statement untuk menghindari SQL Injection
    if ($stmt = $mysqli->prepare("DELETE FROM dt_masterjb WHERE kd_MasterJB = ?")) {
        $stmt->bind_param('s', $kd_MasterJB);
